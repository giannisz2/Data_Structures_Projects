...
Pointer list_get_at(List list, int pos) {
	ListNode node;
	int i = 0;
	for(node = list_first(list); node != NULL; node = list_next(list,node)) {
		if(i == pos) 
			return node->value;
		i++;
	}
...
void list_remove(List list, ListNode node) {
	...
	for(node2 = list->dummy; node2 != NULL; node2 = list_next(list,node2)) 
		if (node2->next == node) 
...
void list_append(List list, List to_append) {
	if(to_append != NULL) {
		for(ListNode node = list_first(to_append);node != NULL; node = list_next(to_append,node)) 
			list_insert_next(list,list_last(list),node->value);
		free(to_append);
	}
}
