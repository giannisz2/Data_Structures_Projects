...
void rec(BTreeNode node, VisitFunc visit) {
	if(node == NULL)
		return;
...
void set_visit(Set set, VisitFunc visit) {
	...
	/* ΠΡΩΤΗ ΥΛΟΠΟΙΗΣΗ !!!!
		for(SetNode n = set_first(set); n != SET_EOF; n = set_next(set, n)) {
			visit(set_node_value(set, n));
		}
	*/  
	rec(set->root,visit);

	cl2 = clock();
	cle = (double)(cl2 - cl1) / CLOCKS_PER_SEC;
