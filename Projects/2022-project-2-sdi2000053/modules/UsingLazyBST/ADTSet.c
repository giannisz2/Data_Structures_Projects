///////////////////////////////////////////////////////////
//
// Υλοποίηση του ADT Set μέσω AVL
//
///////////////////////////////////////////////////////////
...
static SetNode sort_rec(Set set, SetNode node, Vector vec, int x, int y) {
	if(x > y) {
		return NULL;
	}
	...
	// call node_create so a node and it's children are created
	if(node != NULL) 
		node_destroy(node, set->destroy_value);
	
	node = node_create(vector_get_at(vec, m));

	// repeat recursively for left children
	node->left = sort_rec(set, node->left, vec, x, m - 1);
	// then do the same for the right childs of each node
	node->right = sort_rec(set, node->right, vec, m + 1, y);

	//increase size
	set->size++;
	
	//return root of each node
	return node;
...
Set set_create_from_sorted_values(CompareFunc compare, DestroyFunc destroy_value, Vector values) {
	...
	if(values != NULL && vector_size(values) > 2) {
		set->steps = vector_size(values);

		set->root = sort_rec(set, set->root, values, 0, vector_size(values) - 1);
...
void set_insert(Set set, Pointer value) {
	...
	//if height is bigger than 1, then move all values to a vector and recreate the set with the sort_rec function
	if(height(set->root->left) - height(set->root->right) > 1 || height(set->root->right) - height(set->root->left) > 1) {
		...
		set->root = sort_rec(set, set->root, vec, 0, vector_size(vec) - 1);
		vector_destroy(vec);
	}
