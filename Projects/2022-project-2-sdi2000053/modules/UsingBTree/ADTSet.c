...
static SetNode sort_rec(Set set, BTreeNode node, Vector vec, int x, int y) {
	if(x > y)
		return NULL;
	...
	if(node  != NULL) {
		for(int i = 0; i < MAX_CHILDREN; i++) {
			...
			node->set_nodes[i]->value = vector_get_at(vec, m);

			set->size += 1;
			
			node->children[i] = sort_rec(set, node->children[i+1], vec, x, m - 1);
			return node;
		}
	}
	return NULL;
