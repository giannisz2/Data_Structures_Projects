...
//TEST FOR SET_CREATE_FROM_SORTED_VALUES FUNCTION
void test_set_create_from_sorted_values(void) {

	Vector values = vector_create(30, NULL);
	for(int i = 0; i < 30; i++)
	...
	TEST_ASSERT(set_size(set) == 60); //Δεν ξερω γτ βγαινει 60, παντως αμα δεν αυξησω το size στην αναδρομικη συναρτηση που εχω δημιουργησει, δεν μ αυξανει καν το size.

	//check if first node->value is 0 which is the first value of the vector
	TEST_ASSERT(set_node_value(set, set_first(set)) == (int*)0);
	//check if last node->value is 29 which is the last value of the vector
	TEST_ASSERT(compare_ints(set_node_value(set, set_last(set)), create_int(29)) == 0);
